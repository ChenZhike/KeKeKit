...
- (void)tapGestureDetectedForGesture:(UITapGestureRecognizer *)gesture
{
    if ([self.holeViewDelegate respondsToSelector:@selector(holedView:didSelectHoleAtIndex:)]) {
        ...
        NSUInteger index=[self holeViewIndexForAtPoint:touchLocation];
//        JMHoledView*hv=[self.holes objectAtIndex:index];
//        if ([hv isKindOfClass:[JMCustomRectHole class]]) {
//            JMCustomRectHole *hole = (JMCustomRectHole *)hv;
...
//            }
//        [self.holes removeObject:hv];
        [self.holeViewDelegate holedView:self didSelectHoleAtIndex:index];
