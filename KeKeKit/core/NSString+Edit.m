...
-(NSArray*)seperatedPerGroup:(int)group
{
    ...
    int count=[@(self.length) zhengchuplus:group] ;
    NSUInteger maxindex=length-1;
    for (int i=0; i<count; i++) {
        int location=i*group;
        NSUInteger length=group;
        if ((location+group)>=maxindex) {
            length=maxindex-location;
        }
        NSRange range=NSMakeRange(location, length);
        NSString*nowstr=[self substringWithRange:range];
        [result addObject:nowstr];
    }
    return result;
}
-(NSString*)subStrBetweenStr1:(NSString*)str1 str2:(NSString*)str2
{
    NSInteger start=[self rangeOfString:str1].location+str1.length+1;
    NSInteger end=[self rangeOfString:str2].location-1;
    NSString*result=[self substringWithStartIndex:start endindex:end];
    return result;
}
